...
PALAVRAS_CHAVE = [
    'ataque', 'cibernético', 'hacker', 'vazamento', 'dados', 'ransomware', 
    'phishing', 'golpe', 'invasão', 'falha', 'vulnerabilidade', 'ddos', 'malware'
]

# --- MÓDULO DE IA (Treinamento do Modelo) ---
print("Treinando o modelo de IA...")
textos_treino = [
    "empresa sofre com ataque de ransomware e dados são criptografados e sequestrados",
    "milhões de usuários têm senhas e emails expostos em vazamento de dados massivo",
    "novo golpe de phishing usa nome de banco para roubar informações de cartão de crédito",
    "site do governo fica fora do ar após intenso ataque ddos",
    "especialistas descobrem grave falha de segurança em aplicativo popular",
    "grupo hacker exige resgate em bitcoin após ataque de ransomware",
    "informações pessoais de clientes vazaram após invasão a servidor",
    "cuidado com email falso que tenta te enganar para clicar em link malicioso de phishing",
]
categorias_treino = [
    "Ransomware", "Vazamento de Dados", "Phishing", "Ataque DDoS", 
    "Falha de Segurança", "Ransomware", "Vazamento de Dados", "Phishing"
]
...
try:
    ...
    for nome_fonte, url_rss in FONTES_RSS.items():
        ...
        try:
            ...
            for noticia in noticias:
                ...
                resumo = descricao_tag.text if descricao_tag else "Sem resumo."
                
                texto_para_analise = (titulo + " " + resumo).lower()
            ...
            print(f"Erro ao acessar {nome_fonte}: {e}")
    ...
    if todas_as_noticias_relevantes:
        ...
        if total_noticias > 0:
            categoria_principal, contagem_principal = contagem_categorias.most_common(1)[0]
            percentual = (contagem_principal / total_noticias) * 100
        ...
        wedges, texts, autotexts = ax.pie(contagem_categorias.values(), autopct='%1.1f%%', startangle=90)
        ax.legend(wedges, contagem_categorias.keys(), title="Categorias", loc="center left", bbox_to_anchor=(1, 0, 0.5, 1))
        plt.setp(autotexts, size=10, weight="bold", color="white")
        ax.set_title("Distribuição de Tipos de Incidentes Reportados", fontsize=16, pad=20)
